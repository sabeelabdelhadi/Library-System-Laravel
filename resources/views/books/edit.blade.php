    ...
    <h2 class="mb-4 fw-bold">{{ isset($book) ? 'تعديل الكتاب' : 'إضافة كتاب' }}</h2> 
            <form action="{{ isset($book) ? route('books.update', $book->id) : route('books.store') }}" method="post">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="year" class="form-control" value="{{ $book->year ?? '' }}" placeholder="ادخل سنة النشر" required>
              </div>
              ...
              <div class="mb-4">
                ...
                <input type="text" name="price" class="form-control" value="{{ isset($book) ? (int)$book->price . '$' : '' }}" placeholder="ادخل سعر الكتاب" required>             
               </div>
